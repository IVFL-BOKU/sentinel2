...
S2_vrt_L2Agranule = function(granule, verbose = FALSE) {
  if (nchar(Sys.which("gdalbuildvrt")) == 0) {
    ...
    jp2_10m = list.files(granule, pattern = "_B[0-9]{2}_10m.jp2$", recursive = TRUE, full.names = TRUE)
    jp2_20m = list.files(granule, pattern = "_B[0-9]{2}_20m.jp2$", recursive = TRUE, full.names = TRUE)
    jp2_60m = list.files(granule, pattern = "_B[0-9]{2}_60m.jp2$", recursive = TRUE, full.names = TRUE)
    ...
    filename = file.path(granule, basename(granule))
    system(
      sprintf('gdalbuildvrt -resolution "highest" -separate %s.vrt %s',
      filename,
